...
def main():
    ...
    with tabs[1]:
        ...
        st.dataframe(
            top_suppliers[
                ['name', 'industry', 'location', 'sustainability_score', 'cert_count', 'carbon_footprint', 'cost_element',
                 'onboarding_cost_usd', 'lead_time_days', 'switching_cost_usd']
            ],
    ...
    with tabs[3]:
        ...
        if current_supplier and alternative_supplier and current_supplier != alternative_supplier:
            ...
            fig6 = go.Figure()
            fig6.add_trace(go.Bar(name='Current', x=list(impact_diff.keys()),
                                  y=[current['carbon_footprint'], current['water_usage'], current['waste_production']], marker_color='blue'))
            fig6.add_trace(go.Bar(name='Alternative', x=list(impact_diff.keys()),
                                  y=[alternative['carbon_footprint'], alternative['water_usage'], alternative['waste_production']], marker_color='green'))
