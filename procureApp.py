...
def calculate_sustainability_score(row, weights):
    ...
    score = (weights['carbon']*normalized_carbon +
...
def main():
    ...
    industries = st.sidebar.multiselect("Industry",
                                       options=df['industry'].unique(),
                                       default=list(df['industry'].unique())
                                      )
    locations = st.sidebar.multiselect("Location",
                                      options=df['location'].unique(),
                                      default=list(df['location'].unique())
                                     )
    st.sidebar.header("Lead Time and Cost Filters")

    min_lead_time, max_lead_time = st.sidebar.slider("Select Lead Time Range (days)",
                                                   float(df['lead_time_days'].min()),
                                                   float(df['lead_time_days'].max()),
                                                   (float(df['lead_time_days'].min()),float(df['lead_time_days'].max())))
    min_onboarding_cost, max_onboarding_cost = st.sidebar.slider("Select Onboarding Cost Range (USD)",
    ...
    min_switching_cost, max_switching_cost = st.sidebar.slider("Select Switching Cost Range (USD)",
    ...
    ranking_criteria=st.sidebar.selectbox("Ranking Criteria",[
    ...
    with tabs[0]:
        ...
        # 3. Heatmap of correlation matrix of numeric sustainability metrics
    ...
    with tabs[1]:
        ...
        st.dataframe(top_suppliers[['name','industry','location','sustainability_score','cert_count','carbon_footprint','cost_element','onboarding_cost_usd','lead_time_days','switching_cost_usd']],use_container_width=True)
    ...
    with tabs[2]:
        ...
        # Trend graph 1: Distribution of Recycling Rate
        fig4= px.histogram(scored_df, x='recycling_rate', nbins=20, title='Recycling Rate Distribution')
        st.plotly_chart(fig4, use_container_width=True)

        # Trend graph 2: Average Energy Efficiency by Industry
        energy_scores= scored_df.groupby('industry')['energy_efficiency'].mean().reset_index()
        fig5= px.bar(energy_scores, x='industry', y='energy_efficiency', title='Average Energy Efficiency by Industry')
    ...
    with tabs[3]:
        ...
        col1, col2= st.columns(2)
        with col1:
            current_supplier= st.selectbox("Current Supplier", options=scored_df['name'].tolist(), key="current")
        with col2:
            alternative_supplier= st.selectbox("Alternative Supplier", options=scored_df['name'].tolist(), key="alternative")
        if current_supplier and alternative_supplier and current_supplier!=alternative_supplier:
            ...
            fig6= go.Figure()
            fig6.add_trace(go.Bar(name='Current', x=list(impact_diff.keys()), y=[current['carbon_footprint'], current['water_usage'], current['waste_production']], marker_color='blue'))
            fig6.add_trace(go.Bar(name='Alternative', x=list(impact_diff.keys()), y=[alternative['carbon_footprint'], alternative['water_usage'], alternative['waste_production']], marker_color='green'))
